...
```bash
docker build -f ./.devcontainer/Dockerfile --tag=tx-lang:latest .
docker run -it tx-lang:latest
```
...
```bash
docker build --tag=tx-lang:latest --build-arg GCC_VER=10 --build-arg LLVM_VER=11 .
```
...
```bash
docker build --tag=tx-lang:latest --build-arg USE_CLANG=1 .
```
